...
for i = 1:K_3
    ...
    tau_3D_der = sin_phi*(r_k_i(1)*cos_theta-r_k_i(2)*sin_theta)/v_0;
    ...
    da_theta(i+2) = sin_phi*e_i*(cos_theta*tau_3D_der);
end

for i = K_3+1:K_3+K_1
    r_k_i = r_k(i,:);
    tau_1D = r_k_i.'*e_u/v_0;
    tau_1D_der = sin_phi*(r_k_i(1)*cos_theta-r_k_i(2)*sin_theta)/v_0;
    e_i = exp(-1i*omega_m*tau_1D);
    da_theta(i) = cos_phi*e_i*(cos_theta-sin_theta*tau_1D_der);
end
end
